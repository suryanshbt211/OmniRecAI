...
user_clicks = (
    interactions[interactions["interaction_type"] == "click"]
    .groupby("user_id")
    .size()
...
dataset = dataset.merge(user_views, on="user_id", how="left")
dataset = dataset.merge(user_clicks, on="user_id", how="left")
dataset = dataset.merge(user_purchases, on="user_id", how="left")

dataset = dataset.merge(item_views, on="item_id", how="left")
dataset = dataset.merge(item_clicks, on="item_id", how="left")
dataset = dataset.merge(item_purchases, on="item_id", how="left")
...
dataset["label"] = np.where(
    dataset["interaction_type"] == "purchase",
    1,
    0
)
